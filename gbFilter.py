...
# 	sys.exit(2)
# else:
# 	#extract information from gn file
...
for seqrecord in SeqIO.parse(gbinfile,"genbank"):
	...
	seqInfo += "\n"+seq_accession + "\t" + seq_gi + "\t"+seq_strain+"\t"+seq_country +"\t"+seq_collectdate
	
	seq_len = len(seq_seq)
	if seq_len < 800:
		accessionList +=seq_accession +"\n"

output = open(outfile,"w")
#output.write(seqInfo)
output.write(accessionList)
