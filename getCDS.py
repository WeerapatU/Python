#
#get cds from genbank file
#usage: python getCDS.py your_genbank_file 
#2013-12-31
...
if len(sys.argv) < 3:
	print "Usage: python getCDS.py your_genbank_file outfile_name"
	sys.exit(2)
else:
	...
	seqInfo = ""
	nullItem = "" ## to save those sequence accession that record miss cds item
	for seqrecord in SeqIO.parse(gbinfile,"genbank"):
		...
		print seq_accession
		if cds_id != "null":
			seqInfo += ">" + seq_accession + "\n" + cds_seq +"\n"
		else:
			nullItem += seq_accession + " , "

	output = open(outfile,"w")
	output.write(seqInfo)
	print "##	finished	##" + "\n" +"get CDS from genbank, saved to " + outfile 
	if nullItem != "":
		print  "WARRING: " + nullItem +"are can not find CDS in genbank record"
